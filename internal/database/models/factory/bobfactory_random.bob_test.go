...
func TestRandom_pgtypes_Inet(t *testing.T) {
	t.Parallel()

	val1 := random_pgtypes_Inet(nil)
	val2 := random_pgtypes_Inet(nil)

	if val1 == val2 {
		t.Fatalf("random_pgtypes_Inet() returned the same value twice: %v", val1)
	}
}
