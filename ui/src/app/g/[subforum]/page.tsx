...
export default function GeographicCommunityPage() {
  ...
  const loadSubforumUserContext = async () => {
    try {
      const userData = await authenticateUserForSubforum(`g/${subforumName}`);
      if (userData) {
        login(userData);
      }
    } catch (error) {
  ...
  const loadForum = async () => {
    ...
    try {
      ...
      const response = await subforumsApi.getSubforumDetails('g', subforumName);
      setForum(response);
    } catch (err: unknown) {
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="flex-1 lg:pr-80">
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-2">
              <h1 className="text-3xl font-bold">g/{forum.subforum.name}</h1>
        ...
        >
          <div className="sticky top-8">
            <h2 className="text-lg font-semibold mb-4 text-foreground">g/{forum.subforum.name}</h2>
